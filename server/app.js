...
// 问答及医生数据接口
server.get('/quesansw',(req,res)=>{
  let sql='select c.id,c.subject,c.description,c.vote_numbers,u.docname,u.docavatar,u.docaddress from aichong_quesansw as c LEFT JOIN aichong_doctor as u on doctor_id=u.id'
  pool.query(sql,(error,results)=>{
    if(error) throw error;
    res.send({code:200,message:'查询成功',results:results})
  })
...
server.get('/answers',(req,res)=>{
  let id=req.query.id;
  let sql=`select c.id,subject,content,docname,docavatar,docaddress from aichong_quesansw as c INNER JOIN aichong_doctor AS u on doctor_id=u.id where c.id=? `
  pool.query(sql,[id],(error,result)=>{
    if(error) throw error;
